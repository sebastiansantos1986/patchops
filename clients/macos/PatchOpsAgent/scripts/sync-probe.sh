...
BUILD_DIRECTORY="${TMPDIR:-/tmp}/patchops-agent-sync-probe"
...
xcrun swiftc \
  -swift-version 5 \
  -parse-as-library \
  -o "$BUILD_DIRECTORY/sync-probe" \
  "$PROJECT_DIRECTORY/Sources/Models.swift" \
  "$PROJECT_DIRECTORY/Sources/InventoryCollector.swift" \
  "$PROJECT_DIRECTORY/Sources/APIClient.swift" \
  "$PROJECT_DIRECTORY/Tools/SyncProbe.swift"

"$BUILD_DIRECTORY/sync-probe"
